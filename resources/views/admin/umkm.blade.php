...
<div class="flex min-h-screen">
    ...
    <div class="w-64 bg-green-600 text-white flex flex-col">
        ...
        <nav class="flex-1 py-6 px-4 space-y-2">
            ...
            <a href="{{ url('/admin/umkm') }}" class="block py-2 px-4 rounded bg-green-700">UMKM</a>
        ...
        <header class="bg-white shadow px-6 py-4 border-b">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-gray-600">Daftar usaha mikro, kecil dan menengah</p>
                </div>
        ...
        <main class="p-6">
            <div class="bg-white shadow-md rounded-lg p-4">
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-lg font-semibold">Daftar UMKM</h2>
                    <div class="flex flex-row gap-4">
                        <form method="GET" action="{{ route('admin.umkm') }}" class="flex gap-2">
                            <input type="text" name="search" placeholder="Cari nama umkm" value="{{ request('search') }}"
                            ...
                            @if(request('search'))
                                <a href="{{ route('admin.umkm') }}" class="text-red-500 text-sm ml-2 hover:underline">Reset</a>
                            @endif
                        </form>
                    ...
                    <table class="w-full text-sm text-left text-gray-600">
                        ...
                        @forelse ($umkms as $index => $umkm)
                            <tr class="bg-white border-b">
                                ...
                                <td class="px-4 py-2">{{ $umkm->nama_pemilik_umkm }}</td>
                                <td class="px-4 py-2">{{ $umkm->nama_umkm }}</td>
                                <td class="px-4 py-2">{{ $umkm->no_telp_umkm }}</td>
                                <td class="px-4 py-2">{{ Str::limit($umkm->deskripsi_umkm, 50) }}</td>
                                <td class="px-4 py-2">
                                    @if($umkm->foto_umkm)
                                        <img src="{{ asset('storage/' . $umkm->foto_umkm) }}" class="w-16 h-16 object-cover rounded" />
                                    @else
                                        -
                                    @endif
                                </td>
                                <td class="px-4 py-2 flex gap-2">
                                    <button onclick='openUpdateModal(@json($umkm))' class="text-green-600 hover:text-green-800">
                                        <img src="https://img.icons8.com/plasticine/100/create-new.png" class="w-[30px] h-[30px] min-w-[30px] min-h-[30px]" alt="edit">
                                    </button>
                                    <form action="{{ route('admin.umkm.destroy', $umkm->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?')">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="6" class="text-center py-4 text-gray-500">
                                Tidak ada produk umkm{{ request('search') ? ' untuk pencarian "' . request('search') . '"' : '' }}.
                            </td>
                        </tr>
                        @endforelse
                        </tbody>
...
<script>
    ...
    function openUpdateModal(data) {
        document.getElementById('edit_nama_pemilik').value = data.nama_pemilik_umkm;
        document.getElementById('edit_nama_umkm').value = data.nama_umkm;
        document.getElementById('edit_no_telp').value = data.no_telp_umkm;
        document.getElementById('edit_deskripsi').value = data.deskripsi_umkm;

        const form = document.getElementById('formUpdateUMKM');
        form.action = `/admin/umkm/${data.id}`;
        ...
        if (data.foto_umkm) {
            preview.innerHTML = `
                <div class="mt-2">
                    ...
                    <img src="/storage/${data.foto_umkm}" class="w-24 h-24 object-cover rounded border" alt="Foto UMKM">
                </div>
            `;
        } else {
